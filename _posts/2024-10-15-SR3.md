...
$$\mathcal{D} = { \mathbf{x}_i, \mathbf{y}_i \right}_{i=1}^N \sim p(\mathbf{x}, \mathbf{y})$$ : 미지의 분포에 대한 input-output 짝
...
\begin{split}
    ...
    \text{where} \quad \mathbf{\epsilon} \sim \mathcal{N(\mathbf{0}, \mathbf{I})}, \; (\mathbf{x}, \mathbf{y}) \sim \mathcal{D}, \; p \in \{1,2\}, \: \gamma \sim p(\gamma)
\end{split}
$$
...
Rearaangement 1의 식을 사후분포 $$q(\mathbf{y}_{t-1} \vert \mathbf{y}_0, \mathbf{y}_t)$$ 에 대입하면 다음과 같이 정리할 수 있다.
...
$$
\begin{split}
    \mathbb{E}_{\mathbf{x}, \mathbf{y}_0} \log p_{\theta} (\mathbf{y}_0 \vert \mathbf{x}) \ge \mathbb{E}_{\mathbf{x}, \mathbf{y}_0} \mathbb{E}_{q(\mathbf{y}_{1:T} \vert \mathbf{y}_0)} \left[ \log p(\mathbf{y}_T) + \sum_{t \ge 1} \log \frac{p_{\theta} (\mathbf{y}_{t-1} \vert \mathbf{y}_t, \mathbf{x})}{q(\mathbf{y}_t \vert \mathbf{y}_{t-1})} \right]
\end{split}
$$
