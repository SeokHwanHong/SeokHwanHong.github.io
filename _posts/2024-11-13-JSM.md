...
\begin{aligned}
    ...
    &= argmin_{w,x} \tau \cdot \Psi_{LSM}(w) + \lambda \cdot \Psi_{NLSM} (x) \\
    &+ \frac{\mu}{2} \Vert u^{(k+1)}-w-b^{(k)} \Vert_{2}^2 + \frac{\mu}{2} \Vert u^{(k+1)}-x-c^{(k)} \Vert_{2}^{2} \\
...
$$
\begin{split}
    &1.\text{Initilaize initial value } x_0, \; \text{auxiliary variable} \; y_1 = x_0, \quad \text{acceleration parameter } \; t_1 = 1\\
