...
\begin{split}
    ...
    &\rightarrow \mathbb{E}_{X,Y} [\mathcal{L}(Y, \phi(x)) \vert X = x] = \mathbb{P}(Y \neq \phi(x) \vert X = x) = 1 - \mathbb{P}[Y = \phi(x) \vert X = x]
\end{split}
$$
...
$$
\begin{split}
    \phi^*(x) = argmax_k \; \mathbb{P} (Y = k \vert X = x)
\end{split}
$$

이에 대해 사후확률 $$P_k(x) = \mathbb{P} (Y = k \vert X = x)$$ 와 $$X \vert Y = k$$ 의 확률밀도함수 $$f_k(x)$$ 를 이용해 다음과 같이 표현할 수 있다.

$$
\begin{split}
    P_k(x) = \mathbb{P} (Y = k \vert X = x) \propto \pi_k(x) \cdot f_k(x)
\end{split}
$$
