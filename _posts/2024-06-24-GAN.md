...
$$
\begin{aligned}
    \min_{G} \max_{D} V(D,G) = \mathbb{E}_{\mathbf{x} \sim p_{data}(\mathbf{x})}[\left \log D(\mathbf{x}) ]\right + \mathbb{E}_{\mathbf{z} \sim p_{\mathbf{z}}(\mathbf{z}) }[\left \log 1-D(G(\mathbf{\mathbf{z}}))]\right \\
\end{aligned}
$$
...
$ \mathbb{E}_{\mathbf{z} \sim p_{\mathbf{z}}({\mathbf{z}})} [\left 1-logD(G(\mathbf{z})) ]\right $ : $ \mathbf{z}$ 에서 표본 $z$ 를 뽑아 $1-\log D(G(z))$ 의 기댓값을 계산 
