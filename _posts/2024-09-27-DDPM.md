...
$$
\begin{split}
    \mathbb{E}[-\log p_{\theta}(\mathbf{x}_0)] \le \mathbb{E}_q [ -\log \frac{p_{\theta}(\mathbf{x}_{0:T})}{q(\mathbf{x}_{1:T} \vert \mathbf{x}_0)} ] = \mathbb{E}_q [ -\log p(\mathbf{x}_T) -\sum _{t \ge 1} \log \frac{p_{\theta}(\mathbf{x}_{t-1} \vert \mathbf{x}_t)}{q(\mathbf{x}_{t} \vert \mathbf{x}_{t-1})} ] =: L
\end{split}
$$
...
$$
\begin{split}
    \text{where} \quad \tilde{\mathbf{\mu}}_t (\mathbf{x}_t, \mathbf{x}_0) := \frac{\sqrt{\bar{\alpha}_{t-1}} \beta_t}{1-\bar{\alpha}_t} \mathbf{x}_0 + \frac{\sqrt{\bar{\alpha}}(1-\bar{\alpha}_{t-1})}{1-\bar{\alpha}_t} \mathbf{x}_t \quad and \quad \tilde{\beta}_t := \frac{1-\bar{\alpha}_{t-1}}{1-\bar{\alpha}_t} \beta_t
\end{split}
$$
...
$$
\begin{split}
    L_{t-1} = \mathbb{E}_q [ \frac{1}{2 \sigma_t^2} \Vert \tilde{\mathbf{\mu}}_t(\mathbf{x}_t, \mathbf{x}_{0}) - \mathbf{\mu}_{\theta}(\mathbf{x}_t,t) \Vert^2] + C
\end{split}
$$
...
$$
\begin{split}
    L_{t-1} - C &= \mathbb{E}_{\mathbf{x}_0, \mathbf{\epsilon}} [\frac{1}{2 \sigma_t^2} \Vert \tilde{\mathbf{\mu}}_t (\mathbf{x}_t (\mathbf{x}_0, \mathbf{\epsilon}), \frac{1}{\sqrt{\bar{\alpha}_t}} (\mathbf{x}_t (\mathbf{x}_0, \epsilon) - \sqrt{1-\bar{\alpha}_t} \mathbf{\epsilon} ) ) - \mathbf{\mu}_{\theta} (\mathbf{x}_t (\mathbf{x}_0, \mathbf{\epsilon}), t ) \Vert^2 ]  \\
    &= \mathbb{E}_{\mathbf{x}_0, \mathbf{\epsilon}} [ \frac{1}{2 \sigma_t^2} \Vert \frac{1}{\sqrt{\alpha_t}} (\mathbf{x}_t (\mathbf{x}_0, \mathbf{\epsilon}) - \frac{\beta_t}{\sqrt{1-\bar{\alpha}_t}} \mathbf{\epsilon}) - \mathbf{\mu}_{\theta} (\mathbf{x}_t (\mathbf{x}_0, \mathbf{\epsilon}), t) \Vert^2 ]
\end{split}
$$
...
$$
\begin{split}
    L_{t-1} - C = \mathbb{E}_{\mathbf{x}_0, \epsilon} [\frac{\beta_t^2}{2 \sigma_t^2 \alpha_t (1-\bar{\alpha}_t)} \Vert \mathbf{\epsilon} - \epsilon_{\theta}(\sqrt{\bar{\alpha}_t} \mathbf{x}_0 + \sqrt{1-\bar{\alpha}_t} \mathbf{\epsilon}, t) \Vert^2 ]
\end{split}
$$
